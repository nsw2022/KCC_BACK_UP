...
// FemaleIdol 객체 배열 생성
const femaleIdols = iveMembers.map(member => new FemaleIdol(member.name, member.year));

// 아이돌 그룹 객체 생성
const iveGroup = new IdolGroup('IVE', femaleIdols);

// 나라 객체 생성
const southKorea = new Country('South Korea', [iveGroup]);

console.log(southKorea.idolGroups[0].members.map(idol => idol.dance()));
...
// FemaleIdol 객체 배열 생성
const maleIdol = btsMembers.map(member => new MaleIdol(member.name, member.year));

// 아이돌 그룹 객체 생성
const btsGroup = new IdolGroup('BTS', maleIdol);

// 나라 객체 생성
southKorea.idolGroups.push(btsGroup)

console.log(southKorea.idolGroups[1].members.map(idol => idol.sing()));
